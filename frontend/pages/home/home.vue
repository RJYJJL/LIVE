		...
		methods: {
			...
			async initApiService() {
				try {
					...
					// 从配置文件获取当前服务器地址
					const configUrl = API_BASE_URL || 'http://192.168.43.247:8081';
			...
			try {
				...
				if (streamUrl.includes('.m3u8')) {
					...
					if (hlsUrl.includes('192.168.43.247:8086')) {
						const middlewareServerUrl = 'http://192.168.43.247:8081';
						const originalUrl = hlsUrl;
						hlsUrl = hlsUrl.replace('http://192.168.43.247:8086', middlewareServerUrl);
					}
