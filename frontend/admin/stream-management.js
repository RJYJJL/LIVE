...
async function loadStreamsList() {
	...
	if (Array.isArray(result)) {
		...
		if (streams.length > 0) {
			...
			window.__lastStreamsList = streamsWithDebates;
			renderStreamsTable(streamsWithDebates);
		} else {
			window.__lastStreamsList = streams;
...
function renderStreamsTable(streams) {
	...
	tbody.innerHTML = streams.map(stream => {
		// 获取辩题信息（只显示一行：有辩题显示徽章+标题，无辩题只显示徽章）
		const debateTopic = stream.debateTopic || null;
		const debateTitle = debateTopic ? escapeHtml(debateTopic.title) : '';
		// 需求：删除“已设置”和前面的对号 —— 这里改为仅显示辩题标题，无辩题显示“-”
		const debateCell = debateTopic
			? `<div style="font-size: 12px; color: #333;" title="${debateTitle}">${debateTitle.length > 24 ? debateTitle.substring(0, 24) + '...' : debateTitle}</div>`
		...
		<tr>
			...
			<td>
				<button
					class="btn btn-sm ${stream.enabled ? 'btn-success' : 'btn-secondary'}"
					onclick="event.stopPropagation(); toggleStreamEnabledFromTable('${stream.id}')"
...
async function handleStreamFormSubmit(e) {
	...
	try {
		...
		if (streamData.type === 'hls' && !url.includes('.m3u8')) {
			showToast('HLS 类型请使用 .m3u8 流地址', 'error');
			window.streamFormSubmitting = false;
			return;
		}
		if (streamData.type === 'flv' && !url.includes('.flv')) {
			showToast('FLV 类型请使用 .flv 流地址', 'error');
			window.streamFormSubmitting = false;
			return;
		}
		if (streamData.type === 'rtmp' && !url.startsWith('rtmp://')) {
			showToast('RTMP 类型请使用 rtmp:// 开头地址', 'error');
			window.streamFormSubmitting = false;
			return;
		}
		...
		try {
			...
			if (streamId) {
				...
				// 添加
				console.log('调用添加API');
				const addResult = await addStream(streamData);
				// 若接口返回新建的流，用于乐观更新列表
				addedStream = addResult?.data || addResult?.stream || (addResult && addResult.id ? addResult : null);
			...
			// 乐观更新：若接口返回了新流，先插入到列表最前面，用户立即能看到
			if (addedStream && addedStream.id) {
				const tbody = document.getElementById('streams-table-body');
				if (tbody) {
					const current = (window.__lastStreamsList || []).slice();
					const newList = [addedStream, ...current.filter(s => s.id !== addedStream.id)];
					window.__lastStreamsList = newList;
					renderStreamsTable(newList);
				}
			}
			...
			if (typeof window.dispatchEvent === 'function') {
				window.dispatchEvent(new CustomEvent('streams-list-updated'));
			}
			// 后端写入可能较慢：500ms、1.5s 后再各刷新一次，确保新项出现在列表中
			setTimeout(async () => {
				await loadStreamsList();
				if (typeof window.dispatchEvent === 'function') {
					window.dispatchEvent(new CustomEvent('streams-list-updated'));
...
async function handleDebateFormSubmit(e) {
	...
	try {
		...
		try {
			// 先写入本地，确保列表修改后立即可见（不依赖后端）
			if (typeof saveStreamDebateLocal === 'function') {
				await saveStreamDebateLocal(streamId, debateData).catch(() => {});
			}
			...
			if (debateId) {
				...
				if (result && (result.success !== false)) {
					...
					showToast('辩题已保存到本地', 'success');
				}
			} else {
				// 新建辩题，尝试同步到后端（失败也不影响本地已保存）
				console.log('创建新辩题并关联到流:', streamId);
				const createResult = await createDebate(debateData).catch(() => null);
				const newDebateId = createResult?.data?.id || createResult?.id;
				if (newDebateId) {
					const associateResult = await associateDebateToStream(streamId, newDebateId).catch(() => null);
...
/**
 * 列表内启用/禁用切换（与编辑页“是否启用”保持一致）
 */
async function toggleStreamEnabledFromTable(streamId) {
	try {
		if (typeof toggleStream !== 'function') {
			showToast('系统错误：toggleStream API 未加载', 'error');
			return;
		}
		await toggleStream(streamId);
...
window.toggleStreamEnabledFromTable = toggleStreamEnabledFromTable;
